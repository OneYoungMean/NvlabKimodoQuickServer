...
if exist "%NETWORK_PROBE_PS1%" (
  ...
  if not errorlevel 1 (
    ...
    if defined KIMODO_PIP_INDEX_URL set "PIP_INDEX_URL=%KIMODO_PIP_INDEX_URL%"
    if defined KIMODO_PIP_TRUSTED_HOST set "PIP_TRUSTED_HOST=%KIMODO_PIP_TRUSTED_HOST%"
    if defined KIMODO_PY_ZIP_BASE (
      set "PY_ZIP_MIRROR_BASE=%KIMODO_PY_ZIP_BASE%"
      if /I "%KIMODO_PY_ZIP_BASE%"=="https://www.python.org/ftp/python/" (
        set "PY_ZIP_OFFICIAL_BASE=https://mirrors.tuna.tsinghua.edu.cn/python/"
      ) else (
        set "PY_ZIP_OFFICIAL_BASE=https://www.python.org/ftp/python/"
      )
    )
...
if defined KIMODO_BUILDENV_ONLY (
  echo [INFO][setup_kimodo_offline_impl.bat] BuildEnv-only mode enabled. Skip model clone/check and setup sentinel write.
...
:buildenv_only_done
echo [OK][setup_kimodo_offline_impl.bat] Build environment staged.
